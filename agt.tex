...
Because you always can refer to my books, in this short intro I present theorems without proofs.
...
Note that $\subsets\mho$ (\emph{improper filter}) is also considered a filter by me.
...
\begin{defn}
I order the set~$\mathfrak{F}$ of filters \emph{reverse} to set-theoretic order, that is
...
Note that the set of principal filters is a subset of the set of filters on some set. This suggests the idea of filtrators: A \emph{filtrator} is a pair of a poset and its subset. Filters are a special case of elements of filtrators. Filtrators were first studied by me in~\cite{filters} and \cite{volume-1-edition1} contains a more detailed reference on filtrators.
...
(I will denote the source of a funcoid~$f$ as~$\Src f$ and its destination as~$\Dst f$.)

Quadruple $(A,B,\alpha,\beta)$ of sets~$A$,~$B$ (source and destination of the funcoid) and functions $\alpha:\mathfrak{F}(A)\to\mathfrak{F}(B)$,~$\beta:\mathfrak{F}(B)\to\mathfrak{F}(A)$, conforming to the formula:
\[ \alpha (\mathcal{X})\sqcap \mathcal{Y}\neq \bot \Leftrightarrow \beta (\mathcal{Y})\sqcap \mathcal{X}\neq \bot. \]

\begin{rem}
Funcoid~$(A,B,\alpha,\beta)$ is determined by the value of~$\alpha$ (or value of~$\beta$).
\end{rem}
...
I remind that proximities are an axiomatization of the relation of ``nearness'' (informally, being at zero distance) of two sets.
...
\begin{prop}
A funcoid~$f$ is uniquely determined by~$\supfun{f}$ and moreover by values of~$\supfun{f}$ on ultrafilters or by the relation~$\suprel{f}$ between ultrafilters.
\end{prop}
...
f\bigcup S=\bigsqcup\rsupfun{\rsupfun f}S$;
...
(topological space, (quasi)uniforms space or reloid, (quasi)proximity space of funcoid, (quasi)metric space) is representable as an element of an ordered semigroup action. Both spaces and binary relations (including functions) are interspaces.
...
Why do we consider all shifts of our infinitely small rectangle? To make the limit not dependent of the point~$a$ to which $x$~tends. Otherwise the limit would depend on the point~$a$.
...
(ordered semigroup actions), but that is not yet thoroughly researched yet.
...
Let $\operatorname{SNG}(R)$ be the set of singularities for
a set~$R=\Ob\nu$ for some funcoid~$\nu$.
...
on~$\operatorname{SUPER}(R)$. That is differential equations
...
My book~\cite{volume-1-edition1} also considers more topological properties of funcoids and reloids, pointfree generalizations of funcoids,
